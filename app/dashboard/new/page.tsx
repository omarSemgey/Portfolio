...
export default function NewProjectPage() {
  ...
  // New State variables mapping to backend schema extensions
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        isPersonal, // Attached boolean identifier
        skills,      // Attached string[] array payload
        pictures,
        videos,
      };
      ...
      if (res.ok) {
        ...
        alert(`Creation error: ${errData.error || 'Server rejected request'}`);
      }
    } catch (err) {
      console.error(err);
      alert('Network error compiling transaction.');
    } finally {
    ...
    <main className="min-h-screen bg-main text-default p-6 md:p-12">
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <span className="text-xs font-mono uppercase tracking-widest text-muted">Create New Project</span>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 bg-surface p-6 md:p-8 rounded-xl border border-surface/50 shadow-xl">
          ...
          {/* New Project Classification Block */}
          ...
          {/* New Engine Stack / Skills Manager Block */}
